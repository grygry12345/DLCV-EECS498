...
class RPNPredictionNetwork(nn.Module):
    ...
    def forward(self, feats_per_fpn_level: TensorDict) -> List[TensorDict]:
        ...
        for key, value in feats_per_fpn_level.items(): # {p3:feature tensor,...}
          ...
          boxreg_deltas[key] = boxreg_deltas[key].contiguous().view(batch_size, -1, 4)
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
...
class RPN(nn.Module):
    ...
    def predict_proposals(
        ...
        for level_name in anchors_per_fpn_level.keys():
            ...
            # Get anchor boxes and predictions from a single level. {HWA, 4}
            ...
            for _batch_idx in range(level_obj_logits.shape[0]):
                ...
                ##############################################################
                # Replace "pass" statement with your code
                batch_boxreg_deltas = level_boxreg_deltas[_batch_idx]
                proposals = rcnn_apply_deltas_to_anchors(batch_boxreg_deltas, level_anchors)
                ...
                batch_obj_logits = level_obj_logits[_batch_idx]
                topk = min(self.pre_nms_topk, batch_obj_logits.shape[0])
                topk_obj_logits, topk_idx = torch.topk(batch_obj_logits, topk)
                ...
                topk = min(self.post_nms_topk, post_nms_proposals.shape[0])
                topk_post_nms_proposals = post_nms_proposals[:topk]
                level_proposals_per_image.append(topk_post_nms_proposals)
                ##############################################################
                #                        END OF YOUR CODE                    #
                ##############################################################
...
class FasterRCNN(nn.Module):
    ...
    def __init__(
        ...
        cls_pred.append(nn.Linear(prev_out * roi_size[0] * roi_size[1], num_classes + 1))
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
    ...
    def forward(
        ...
        # Get batch size from FPN feats:
        num_images = feats_per_fpn_level["p4"].shape[0]
        ...
        for level_name in feats_per_fpn_level.keys():
            ...
            ##################################################################
            level_feats = feats_per_fpn_level[level_name] # (B, FPN_channel, H, W)
            level_props = output_dict["proposals"][level_name] # (mix_gt_prop, 4) * batch
        ...
        # shape: (batch_size * total_proposals, num_classes + 1)
    ...
    def inference(
        ...
        scores = F.softmax(pred_cls_logits, dim=0)
