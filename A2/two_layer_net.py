...
def nn_forward_backward(
    ...
    ############################################################################
    # Replace "pass" statement with your code
    scores = scores - scores.max(-1, True).values # stability
    softmax_log = scores - scores.logsumexp(-1, True)
    loss = -softmax_log[range(N), y].mean() 
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    softmax = softmax_log.exp()
    one_hot = torch.zeros_like(torch.empty(N, softmax.shape[1]), device=X.device, dtype=X.dtype)
    one_hot[range(N), y] = 1
    mask = (softmax - one_hot) / N # (N, C)
    grads['b2'] = mask.sum(0) # (C, )
    grads['W2'] = torch.matmul(h1.T, mask)  + reg * 2 * W2  # (H, C)
    mask2 = torch.matmul(mask, W2.T) * (h1 > 0) # (N, H)
    grads['b1'] = mask2.sum(0) # (H, 0)
    grads['W1'] = torch.matmul(X.T, mask2) + reg * 2 * W1 # (D, H)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def find_best_net(
    ...
    lr_, hs_, rs_, lrd_ = get_param_set_fn()
    for lr in lr_:
        for hs in hs_:
            for rs in rs_:
                for lrd in lrd_:
